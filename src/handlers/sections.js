...
module.exports = {
  ...
  async shuffle_block(args) {
    ...
    if (target.type === 'template') {
      await apiFetch(`/api/v1/templates/${encodeURIComponent(target.id)}`, { method: 'PUT', body: { content: tb.nodes } });
      resultText = `Template "${target.id}" updated.`;
    } else {
      const put = await apiFetch(`/api/v1/sites/${encodeURIComponent(target.id)}`, { method: 'PUT', body: { content: tb.nodes } });
